...
int main (int argc, char** argv) {
  ...
  hsize_t Nlocal2[2] = {NX/dim[0], NY/dim[1]};
  hsize_t offset[2] = {mpirank / dim[0], mpirank % dim[0]};
  for(int i=0; i<2; i++) offset[i] *= Nlocal[i];
  hsize_t count[2] = {2,2};
  hsize_t stride[2] = {Nlocal2[0],Nlocal2[1]};
  hid_t localspace = H5Screate_simple(2, Nlocal2, NULL);
  H5Sselect_hyperslab(globalspace, H5S_SELECT_SET, offset, stride, count, Nlocal);
  H5Pclose(plist);
  vector<int> buffer(Nlocal2[0]*Nlocal2[1]);
  ...
  int sum = 0;
  for (int i=0; i<Nlocal2[0]*Nlocal2[1]; i++)
    sum += buffer[i];
  printf("sum=%d\n",sum);
